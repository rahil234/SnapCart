...
const OrdersTable: React.FC<{ orders: IOrder[], onViewDetails: (order: IOrder) => void }> = ({ orders, onViewDetails }) => {
    const queryClient = useQueryClient()

    const updateOrderMutation = useMutation<unknown, Error, UpdateOrderData>({
        mutationFn: (data: UpdateOrderData) => orderEndpoints.updateOrderStatus(data),
        onSuccess: () => {
...
function AdminOrders() {
    ...
    const { data: orders, isLoading, isError } = useQuery<IOrder[]>({
        ...
        queryFn: orderEndpoints.getSellerOrders,
    })
    ...
    const handleUpdateStatus = (orderId: string, newStatus: string) => {
        updateOrderMutation.mutate({ orderId, status: newStatus as UpdateOrderData['status'] })
    }
