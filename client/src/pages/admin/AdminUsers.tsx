...
import userEndpoint from '@/api/userEndpoints';
import {
  Ban,
  ChevronLeft,
  ChevronRight,
...
const AdminUsers = () => {
  ...
  const handleBlockUser = async (userId: string) => {
    try {
      await userEndpoint.blockUser(userId);
      setUsers((prevUsers) =>
        prevUsers.map((user) =>
  ...
  const handleAllowUser = async (userId: string) => {
    try {
      await userEndpoint.allowUser(userId);
      setUsers((prevUsers) =>
        prevUsers.map((user) =>
                      ...
                      <AlertDialogTrigger >
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <span
                              ...
                              onClick={() => {
                                ...
                                setActionType(user.status === 'Active' ? 'block' : 'allow');
                              }}
                            >
                              {user.status === 'Blocked' ? (
                          ...
                          <TooltipContent className="bg-white text-black shadow-lg">
                            <p>{user.status === 'Blocked' ? 'Allow user' : 'Block user'}</p>
                          </TooltipContent>
                        </Tooltip>
                      </AlertDialogTrigger>
