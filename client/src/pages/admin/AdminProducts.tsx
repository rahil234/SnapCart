              ...
              {varientGroup.products.map((product, index) => (
                <tr key={index}>
                  {index === 0 && (<>
                    ...
                    <td className="px-2 py-4 whitespace-nowrap text-xs text-center" rowSpan={varientGroup.products.length}>{`${product.category}/ ${product.subcategory}`}</td>
                  </>)}
...
export default function AdminProducts() {
  ...
  useEffect(() => {
    (async () => {
      const data = (await productEndpoints.getAdminProducts()).data;
